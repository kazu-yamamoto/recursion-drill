...
よって、`<nat>` は一桁の数字を表現し、`<expr_simple>` は一桁の数字の足し算を表す。
...
次に my_expr_simple を定義する。横方向に再帰するだけなので、それほど難しくはない。
...
BNF の `<expr_simple>` と照らし合わせて理解して欲しい。

コードを簡潔にするために補助関数 my_first が定義してある。my_first は、第一引数に取った関数を、第二引数に取った組の「左側の要素」に適応する。このドリルで、はじめて case of という構文が出てきたことに注意。
